...
void systime_periodic(void)
{
	for (uint8_t i = 0; i < ARRAY_SIZE(systime_timer); i++) {
		...
		if (systime_localtime - systime_timer[i].time_last > systime_timer[i].period) {
			if (systime_timer[i].type == 0) {
				if (systime_timer[i].fnc) {
					systime_timer[i].fnc();
				}
			} else {
				if (systime_timer[i].fnc_arg) {
					systime_timer[i].fnc_arg(systime_timer[i].arg);
				}
			}
...
void sys_untimeout(timer_fnc_arg fnc, void *arg)
{
	uint8_t i;

	for (i = 0; i < ARRAY_SIZE(systime_timer); i++) {
		if (systime_timer[i].active == 1) {
			if (systime_timer[i].type == 0) continue;

			if ((systime_timer[i].fnc_arg == fnc) && (systime_timer[i].arg == arg)) {
				systime_timer[i].active = 0;
			}
		}
	}
...
void sys_timeout(uint32_t ms, timer_fnc_arg fnc, void * arg)
{
	uint8_t i;
	...
	systime_timer[i].fnc_arg = fnc;
	systime_timer[i].arg = arg;
	systime_timer[i].period = (ms + SYSTIME_PERIOD_MS-1)/SYSTIME_PERIOD_MS;
	systime_timer[i].time_last = systime_localtime;

	systime_timer[i].oneshot = 1;
	systime_timer[i].active = 1;
	systime_timer[i].type = 1;
}
