...
static uint8_t cmd_exit(struct console_session_t *cs, char **args);
...
struct console_command_t console_command[] = {
	...
	{ "exit",    cmd_exit, "Close session", "Close:\n closes session", NULL },
};
...
static uint8_t cmd_reboot(struct console_session_t *cs, char **args)
{
	...
	if (args[0] != NULL) {
		if (strcmp(args[0], "boot") == 0) {

#ifdef STM32F1
			BKP->DR1 = (MAGIC_KEY >> 16) & 0xffff;
			BKP->DR2 = MAGIC_KEY & 0xffff;
#endif
#ifdef STM32F417VG
			RTC->BKP0R = MAGIC_KEY;
#endif

			len = tfp_sprintf(out, "Rebooting into bootloader\n");
			console_session_output(cs, out, len);
		}
#if 0
		if (strcmp(args[0], "boot") == 0) {
...
#endif
#ifdef STM32F417VG
			RTC->BKP0R = MAGIC_KEY_L2;
#endif

			len = tfp_sprintf(out, "Rebooting into bootloader l2\n");
			console_session_output(cs, out, len);
		}
#endif
	}
